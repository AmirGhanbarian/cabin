...
export function Contact() {
  ...
  const validate = () => {
    ...
    if (!form.phone.trim()) e.phone = t.contact.phoneError;
    else if (form.phone.replace(/\D/g, '').length < 7) e.phone = t.contact.phoneError;
    setErrors(e);
  ...
  return (
    <section id="contact" className="section-pad bg-ink-900 relative overflow-hidden">
        ...
        <div className="grid gap-12 lg:grid-cols-2 lg:gap-16">
          ...
          <Reveal>
            <div>
              ...
              <h2 className="mt-3 font-serif text-heading text-cream-50 text-balance">
                {t.contact.title}
              </h2>
              <p className="mt-4 max-w-md text-base leading-relaxed text-cream-300">
              ...
              <div className="mt-10 space-y-4">
                <a
                  href="tel:+15550100"
                  className="group flex items-center gap-4 rounded-2xl border border-cream-200/10 bg-cream-50/5 p-5 transition-all hover:border-brass-400/40 hover:bg-cream-50/10"
                >
                  ...
                  <div>
                    ...
                    <div className="font-serif text-lg font-semibold text-cream-50">
                      +1 555-0100
                    </div>
                  </div>
                </a>
                ...
                <a
                  href="mailto:hello@rufcabinetry.com"
                  className="group flex items-center gap-4 rounded-2xl border border-cream-200/10 bg-cream-50/5 p-5 transition-all hover:border-brass-400/40 hover:bg-cream-50/10"
                >
                  <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-brass-500/20 text-brass-400 transition-colors group-hover:bg-brass-500 group-hover:text-ink-900">
              ...
              {status === 'success' ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <button
                    ...
                    className="mt-8 rounded-full border border-ink-200 px-6 py-2.5 text-sm font-medium text-ink-700 transition-colors hover:bg-ink-100"
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full rounded-xl border px-4 py-3 text-ink-900 placeholder:text-ink-300 transition-colors focus:outline-none focus:ring-2 ${
                        errors.name
                          ? 'border-red-400 focus:ring-red-200'
                          : 'border-ink-200 focus:border-brass-400 focus:ring-brass-200'
                      }`}
                    />
                    {errors.name && (
                  ...
                  <div>
                    <label htmlFor="email" className="mb-1.5 block text-sm font-medium text-ink-700">
                      {t.contact.email} <span className="text-ink-400">{t.contact.emailOptional}</span>
                    </label>
                    <input
                      id="email"
                      type="email"
                  ...
                  <div>
                    <label htmlFor="message" className="mb-1.5 block text-sm font-medium text-ink-700">
                      {t.contact.message} <span className="text-ink-400">{t.contact.emailOptional}</span>
                    </label>
                    <textarea
                      id="message"
                      rows={3}
                  ...
                  <button
                    ...
                    className="flex w-full items-center justify-center gap-2 rounded-full bg-brass-500 px-8 py-4 text-base font-medium text-ink-900 shadow-lg shadow-brass-500/20 transition-all hover:scale-[1.01] hover:bg-brass-400 disabled:opacity-60 disabled:hover:scale-100"
                  >
                    {status === 'loading' ? (
                      <>
