...
import { Heart, ArrowLeft, ShoppingBag, Bell, X, Check } from 'lucide-react';
import { useLang } from '@/lib/lang-context';
import { navigate } from '@/lib/router';
import { supabase, type Product, type Category, type NotifyRequestInsert } from '@/lib/supabase';
...
export function ShopPage() {
  ...
  const handleNotifySubmit = async () => {
    ...
    const payload: NotifyRequestInsert = {
      product_id: notifyProduct.id,
      email: notifyEmail,
    };
    await supabase.from('notify_requests').insert(payload);
    ...
    <div className="pt-20 lg:pt-24">
      ...
      <div className="sticky top-16 z-30 border-b border-ink-100 bg-cream-100/90 backdrop-blur-md lg:top-20">
        <div className="container-px py-4">
          <div className="flex gap-1 overflow-x-auto rounded-full bg-cream-200 p-1">
            <button
        ...
        <div className="container-px">
          {loading ? (
            ...
            <div className="py-20 text-center">
              <p className="text-ink-400">{t.shop.noProducts}</p>
            </div>
          ) : (
                    ...
                    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-ink-100 bg-cream-50 transition-all duration-300 hover:border-brass-300 hover:shadow-xl hover:shadow-brass-500/10">
                      <div className="aspect-square overflow-hidden">
                        <img
                          ...
                          alt={lang === 'en' ? product.name_en : product.name_fa}
                      ...
                      <div className="flex flex-1 flex-col p-5 lg:p-6">
                        ...
                        <h3 className="mt-3 font-serif text-xl font-bold text-ink-900">
                          {lang === 'en' ? product.name_en : product.name_fa}
                        </h3>
                        <p className="mt-2 text-sm leading-relaxed text-ink-500 line-clamp-2">
                          {lang === 'en' ? product.description_en : product.description_fa}
                        </p>
                        <p className="mt-3 text-lg font-bold text-brass-600">
                        ...
                        {inStock ? (
                          <button
                            ...
                            className="flex items-center justify-center gap-2 rounded-full bg-ink-900 px-6 py-3 text-sm font-medium text-cream-100 transition-all hover:bg-ink-800"
                          >
                          ...
                          <button
                            onClick={() => { setNotifyProduct(product); setNotifyDone(false); }}
                            className="flex items-center justify-center gap-2 rounded-full border border-brass-300 px-6 py-3 text-sm font-medium text-brass-600 transition-all hover:bg-brass-50"
                          >
          ...
          <div className="w-full max-w-md rounded-3xl bg-cream-50 p-6 lg:p-8">
            {notifyDone ? (
              <div className="flex flex-col items-center text-center">
                ...
                <button
                  onClick={closeNotify}
                  className="mt-6 rounded-full bg-ink-900 px-6 py-3 text-sm font-medium text-cream-100 transition-colors hover:bg-ink-800"
                >
                  {t.shop.notifyCancel}
                </button>
              </div>
              ...
              <>
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <button onClick={closeNotify} className="rounded-full p-2 text-ink-400 transition-colors hover:bg-ink-100">
                    <X className="h-5 w-5" />
                  </button>
                </div>
                <p className="mb-4 text-sm text-ink-500">
                  {lang === 'en' ? notifyProduct.name_en : notifyProduct.name_fa}
                </p>
                <input
                  type="email"
                  value={notifyEmail}
                  onChange={(e) => setNotifyEmail(e.target.value)}
                  placeholder={t.shop.notifyEmailPlaceholder}
                ...
                <div className="mt-6 flex gap-3">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 rounded-full bg-brass-500 px-6 py-3 text-sm font-medium text-ink-900 transition-colors hover:bg-brass-400 disabled:opacity-60"
                  >
                    {notifySubmitting ? (
